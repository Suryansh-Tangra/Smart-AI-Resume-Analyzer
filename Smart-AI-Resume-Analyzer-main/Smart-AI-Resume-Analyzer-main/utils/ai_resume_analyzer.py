...
class AIResumeAnalyzer:
    def __init__(self):
        ...
        self.azure_ai_api_key = os.getenv("AZURE_AI_API_KEY") or os.getenv("QWEN_API_KEY")
        self.azure_ai_endpoint = os.getenv("AZURE_AI_ENDPOINT") or "https://resumebuilder987.services.ai.azure.com/openai/v1/chat/completions"
        self.azure_ai_model = os.getenv("AZURE_AI_MODEL") or "qwen--qwen35-4b"
    ...
    def analyze_resume_with_anthropic(self, resume_text, job_description=None, job_role=None):
        """Analyze resume using Anthropic Claude via OpenRouter."""
        if not resume_text:
            return {"error": "Resume text is required for analysis."}
        ...
        try:
            ...
            response = requests.post(
                ...
                headers={
                    ...
                    "X-Title": "Smart AI Resume Analyzer",
                },
                json={
            ...
            if isinstance(content, list):
                analysis = "".join(part.get("text", "") for part in content if isinstance(part, dict))
            else:
    ...
    def analyze_resume_with_qwen(self, resume_text, job_description=None, job_role=None):
        """Analyze resume using the Azure AI Qwen endpoint."""
        if not resume_text:
            return {"error": "Resume text is required for analysis."}

        api_key = self.azure_ai_api_key
        endpoint = self.azure_ai_endpoint
        model_name = self.azure_ai_model

        if not api_key:
            return {"error": "Azure AI API key is not configured. Please add AZURE_AI_API_KEY to your .env file."}

        if not endpoint:
            return {"error": "Azure AI endpoint is not configured. Please add AZURE_AI_ENDPOINT to your .env file."}
        ...
        try:
            ...
            return {
                ...
                "resume_score": resume_score,
                "ats_score": ats_score,
                "model_used": "Qwen 3.5 4B",
            }

        except Exception as e:
            return {"error": f"Qwen Azure AI analysis failed: {str(e)}"}
    ...
    def analyze_resume(self, resume_text, job_role=None, role_info=None, model="Google Gemini"):
        ...
        - model: The AI model to use ("Google Gemini", "Anthropic Claude", or "Qwen 3.5 4B")
        ...
        try:
            ...
            elif model in ("Qwen 3.5 4B", "qwen--qwen35-4b"):
                result = self.analyze_resume_with_qwen(resume_text, job_description, job_role)
                model_used = result.get("model_used", "Qwen 3.5 4B")
            else:
