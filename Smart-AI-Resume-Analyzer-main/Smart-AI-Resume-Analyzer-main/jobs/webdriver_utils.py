        ...
        try:
            ...
            for path in chrome_paths:
                if os.path.exists(path):
                    try:
                        # Try using WMIC to get version.
                        # Normalize the path before putting it into the command,
                        # avoiding a backslash inside an f-string expression.
            ...
            if result.returncode == 0:
                ...
                for line in result.stdout.split("\n"):
                    if "Chromedriver path:" in line:
                        chromedriver_path = line.split(
                            "Chromedriver path:"
                        )[1].strip()
                        return chromedriver_path
            else:
                st.warning(
                    f"Chromedriver setup failed: {result.stderr}"
                )
        else:
...
def setup_webdriver():
    ...
    # Method 1: Try direct initialization first
    try:
